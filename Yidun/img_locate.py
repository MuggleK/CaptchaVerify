...
def _pic_download(url, type, ip):
    ...
    img_path = save_path + '/{}_{}.png'.format(type, str(uuid.uuid1()))
    img_data = requests.get(url, proxies=ip, verify=False).content
...
def _get_distance(slider_url, captcha_url, ip):
    ...
    # 引用上面的图片下载
    slider_path = _pic_download(slider_url, 'slider', ip)

    # 引用上面的图片下载
    captcha_path = _pic_download(captcha_url, 'captcha', ip)
    img_size = Image.open(captcha_path).size

    distance = GapLocater(slider_path, captcha_path, save_path + '/slide_result.jpg').run(True)[0] + 3
