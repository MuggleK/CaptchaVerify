...
lib = CDLL(file_path+"/captcha/libdarknet.so", RTLD_GLOBAL) # 这里需要改为你自己的路径
...
def detect(net, meta, image, thresh=.5, hier_thresh=.5, nms=.45):  
    ...
    print('nbox:', num, "thresh:", nms)
    #print_detector_detections2("zhongwen", dets, num, meta.classes, im.w, im.h)
    res = []

    for j in range(num):
        #print("classes: %d" %meta.classes)
        for i in range(meta.classes):
            if dets[j].prob[i]:
                b = dets[j].bbox
                res.append((meta.names[i], dets[j].prob[i], (b.x, b.y, b.w, b.h)))
                # print("***", meta.names[i], dets[j].prob[i], (b.x, b.y, b.w, b.h))
    res = sorted(res, key=lambda x: -x[1])
    free_image(im)
